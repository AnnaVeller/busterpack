...
function _openBusterpack(busterpack) {
    ...
    var notEnoughEqPreLast = busterpack.checkIfAllEqInStore(2).slice(0, 3); // take only 3 or less equipment
    for (var i = 0; i < notEnoughEqPreLast.length; i++) {
        busterpack.equipment[i] = notEnoughEqPreLast[i];
    }
    var notEnoughEqLast = busterpack.checkIfAllEqInStore(3); // take all remain equipment
